...
const getAllVideos = asyncHandler(async (req, res) => {
  ...
  const video = await Video.aggregate([
    // {
    //   $match: {
    //     isPublished: true,  if you want to fetch only published videos
    //   },
    // },
    {
      $lookup: {
    ...
    const video = await Video.aggregate([
      {
        $match: {
          ...
          //  isPublished: true,   // Uncomment if you want to ensure the video is published
         },
      },
      {
        $lookup: {
...
const videoViews = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { videoStatus }, "Video views updated successfully")
    );
});
